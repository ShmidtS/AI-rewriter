...
MIN_WORDS_FOR_DUPLICATE_CHECK = 10
...
def calculate_text_quality_metrics(original: str, rewritten: str) -> Dict[str, float]:
    """
    Вычисляет метрики качества переписанного текста с оптимизацией для производительности.
    ...
    Returns:
        Словарь с метриками: similarity, length_ratio, diversity
    """
    metrics = {}
    ...
    # Оптимизация: используем более быстрые регулярные выражения
    ...
    # Оптимизация: уменьшаем нагрузку на SequenceMatcher для длинных текстов
    if orig_len > 10000 or rew_len > 10000:
        # Для длинных текстов используем упрощенный алгоритм
        sample_size = min(5000, orig_len)
        similarity = difflib.SequenceMatcher(
            None, original[:sample_size], rewritten[:sample_size]
        ).ratio()
    else:
...
def find_split_point(
    text: str, start: int, target_end: int, min_len: int, max_len: int
) -> int:
    """
    Функция поиска точки разбиения.
    """
    text_len = len(text)
...
def create_fallback_prompt(
    ...
    return f"""EDUCATIONAL WRITING EXERCISE: Text paraphrasing and stylistic improvement
...
def validate_rewritten_text(
    ...
    """
    Валидация переписанного текста с метриками качества.
    ...
    if original.strip() and text_cleaned.strip() and original != text_cleaned:
        ...
        if similarity >= SIMILARITY_THRESHOLD:
            return (
                False,
                f"{context}: Слишком похож на оригинал ({similarity:.3f}) после очистки маркеров.",
                quality_metrics,
            )
        ...
        if similarity > 0.85:
            logger.warning(
                f"{context}: Высокая схожесть ({similarity:.3f}), но в пределах допустимого."
            )
    ...
    if orig_len > 0:
        ...
        if text_len_cleaned > max_len:
            return (
                False,
                f"{context}: Слишком длинный ({text_len_cleaned} > {int(max_len)}) после очистки маркеров.",
                quality_metrics,
            )
        ...
        if text_len_cleaned < min_len and orig_len > 20:
            return (
                False,
                f"{context}: Слишком короткий ({text_len_cleaned} < {int(min_len)}) после очистки маркеров.",
                quality_metrics,
            )

    # Улучшенная проверка на повторение предложений из контекста
    rewritten_sentences = [
        normalize_sentence(s) for s in split_into_sentences(text_cleaned)
    ]
    ...
    if prev_block:
        ...
        if repeated_prev:
            logger.debug(
                f"Повтор предложений из предыдущего блока: {repeated_prev[:2]}..."
            )
            return (
                False,
                f"{context}: Содержит предложение(я) из предыдущего блока.",
                quality_metrics,
            )
    ...
    if next_block:
        ...
        if repeated_next:
            logger.debug(
                f"Повтор предложений из следующего блока: {repeated_next[:2]}..."
            )
            return (
                False,
                f"{context}: Содержит предложение(я) из следующего блока.",
                quality_metrics,
            )
    ...
    if quality_metrics["diversity"] < 0.1 and orig_len > 50:
        logger.warning(
            f"{context}: Низкое разнообразие лексики ({quality_metrics['diversity']:.3f})"
        )
...
def calculate_adaptive_temperature(
    failed_attempts: int, quality_metrics: Optional[Dict[str, float]] = None
) -> float:
    """
...
def save_state(filename: str, data: Dict):
    """
    Сохраняет состояние процесса переписывания.
    """
    ...
    try:
        ...
        logger.debug(f"Состояние сохранено: {filename}")
    except Exception as e:
        logger.error(f"Ошибка сохранения состояния в {filename}: {e}", exc_info=True)
        ...
        try:
            ...
            logger.warning(
                f"Не удалось удалить временный файл {temp_file}: {cleanup_error}"
            )
        raise
...
def load_state(filename: str) -> Optional[Dict]:
    """
    Загружает состояние процесса переписывания.
    """
    if os.path.exists(filename):
        try:
            with open(filename, "r", encoding="utf-8") as f:
                state = json.load(f)
            if (
                "processed_block_index" in state
                and "original_blocks_data" in state
                and "total_blocks" in state
            ):
                logger.debug(f"Состояние загружено: {filename}")
                return state
            else:
                logger.warning(f"Файл состояния {filename} имеет неполный формат")
                return None
        except json.JSONDecodeError as e:
            logger.error(f"Ошибка парсинга JSON в файле {filename}: {e}")
            return None
        except Exception as e:
            logger.error(f"Ошибка загрузки состояния из {filename}: {e}", exc_info=True)
            return None
    else:
        logger.debug(f"Файл состояния не существует: {filename}")
        return None


def save_intermediate(filename: str, content: str, context: str = ""):
    """
    Сохраняет промежуточный результат процесса.
    """
    temp_file = filename + ".tmp"
    try:
        with open(temp_file, "w", encoding="utf-8") as f:
            f.write(content)
        os.replace(temp_file, filename)
        logger.debug(f"{context}: Промежуточный файл сохранен: {filename}")
    except Exception as e:
        logger.error(
            f"{context}: Ошибка сохранения промежуточного файла {filename}: {e}",
            exc_info=True,
        )
        # Пытаемся удалить временный файл при ошибке
        try:
...
def rewrite_process(params: Dict, progress_callback=None, stop_event=None):
    ...
    if resume:
        ...
        if state and "original_blocks_data" in state:
            try:
                ...
                logger.warning(
                    f"Ошибка чтения промежуточного файла {intermediate_file}: {e}. Начинаем заново."
                )

    if not state_loaded:
        logger.info(
            "Инициализация нового процесса или перезапуск после ошибки загрузки."
        )
    ...
    current_system_instruction = SYSTEM_INSTRUCTION_BASE.format(
    ...
    for i in range(total_blocks):
        ...
        try:
            ...
            logger.error(
                f"Ошибка: Попытка доступа к блоку с индексом {i}, но всего блоков {total_blocks}."
            )
            break
        ...
        if block.get("processed", False):
            logger.debug(
                f"Блок {i+1}/{total_blocks} помечен как обработанный, пропуск."
            )
            continue
        if block.get("failed_attempts", 0) >= MAX_RETRIES:
            logger.warning(
                f"Блок {i+1}/{total_blocks} пропущен: превышен лимит ({MAX_RETRIES}) неудачных попыток."
            )
            continue
        ...
        if not (0 <= start <= end <= current_rewritten_text_len):
            logger.error(
                f"Ошибка: Некорректные границы для блока {i+1}: start={start}, end={end}, text_len={current_rewritten_text_len}. Прерывание."
            )
            save_state(
                state_file,
                {
        ...
        logger.info(
            f"Обработка блока {i+1}/{total_blocks} [{start}:{end}] (Длина: {end-start} симв.)"
        )
        ...
        if i > 0:
            ...
            if 0 <= prev_start <= prev_end <= current_rewritten_text_len:
                ...
                # else:
                prev_block_text = (
                    full_prev_text  # Если предложений мало, берем весь блок
                )
            else:
                logger.warning(
                    f"Некорректные границы для предыдущего блока {i}, используем пустой контекст."
                )
        ...
        if i < total_blocks - 1:
            ...
            if 0 <= next_start <= safe_next_end <= current_rewritten_text_len:
                ...
                # else:
                next_block_text = (
                    full_next_text  # Если предложений мало, берем весь блок
                )
            else:
                logger.warning(
                    f"Некорректные границы для следующего блока {i+2}, используем пустой контекст."
                )
        ...
        if (
        ...
        ):  # Успешный переписанный и валидированный текст (уже без маркеров)
            new_text_len = count_chars(new_text)
            delta = new_text_len - len(
                block_text
            )  # Изменение длины относительно *оригинального* текста блока
            ...
            logger.error(
                f"Блок {i+1} не был переписан после {block['failed_attempts']} попыток."
            )
        ...
        if save_interval and (
            (new_text is not None and (i + 1) % save_interval == 0)
            or block["failed_attempts"] >= MAX_RETRIES
        ):
    ...
    logger.info(
        f"Переписывание завершено. Обработано: {processed_count}/{total_blocks}. Пропущено из-за ошибок: {failed_count}."
    )

    # Финальное сохранение результата
    final_file_path = os.path.join(output_dir, base_name + FINAL_SUFFIX)
...
class BookRewriterApp:
    def __init__(self, master):
        ...
        ttk.Label(settings_frame, text="Входной файл:").grid(
            row=0, column=0, sticky=tk.W
        )
        self.input_var = tk.StringVar()
        ttk.Entry(settings_frame, textvariable=self.input_var, width=60).grid(
            row=0, column=1, sticky=tk.EW
        )
        ttk.Button(settings_frame, text="Обзор", command=self.browse_input).grid(
            row=0, column=2
        )

        ttk.Label(settings_frame, text="Выходной файл:").grid(
            row=1, column=0, sticky=tk.W
        )
        self.output_var = tk.StringVar()
        ttk.Entry(settings_frame, textvariable=self.output_var, width=60).grid(
            row=1, column=1, sticky=tk.EW
        )
        ttk.Button(settings_frame, text="Обзор", command=self.browse_output).grid(
            row=1, column=2
        )
        ...
        ttk.Checkbutton(
            settings_frame, text="Возобновить", variable=self.resume_var
        ...
        self.start_btn = ttk.Button(
            control_frame, text="Старт", command=self.start_rewrite
        )
        self.start_btn.pack(side=tk.LEFT, padx=5)
        self.stop_btn = ttk.Button(
            control_frame, text="Стоп", command=self.stop_rewrite, state=tk.DISABLED
        )
        self.stop_btn.pack(side=tk.LEFT, padx=5)

        self.status_var = tk.StringVar(value="Готов")
        ttk.Label(control_frame, textvariable=self.status_var, anchor=tk.E).pack(
        ...
        self.log_area = scrolledtext.ScrolledText(
            log_frame, state=tk.DISABLED, height=15
        )
    ...
    def browse_input(self):
        ...
        if file:
            ...
            if not self.output_var.get():
                ...
                self.output_var.set(f"{base}{FINAL_SUFFIX}")
    ...
    def update_progress(self, current: int, total: int):
        if total > 0:
            ...
            self.status_var.set(
                f"Обработка: {current}/{total} ({current/total*100:.1f}%)"
            )
        else:
    ...
    def start_rewrite(self):
        ...
        if not all(
            ...
            messagebox.showerror(
                "Ошибка", "Заполните пути к файлам, стиль и цель переписывания."
            )
            return
        if not os.path.exists(params["input_file"]):
            messagebox.showerror(
                "Ошибка", f"Входной файл не найден:\n{params['input_file']}"
            )
            return
        ...
        self.rewrite_thread = threading.Thread(
            target=rewrite_process,
            args=(params, self.update_progress_threadsafe, self.stop_event),
            daemon=True,
        )
    ...
    def on_closing(self):
        if self.rewrite_thread and self.rewrite_thread.is_alive():
            if messagebox.askyesno(
                "Выход", "Процесс переписывания активен. Остановить его и выйти?"
            ):
                self.stop_rewrite()
