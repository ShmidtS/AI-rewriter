...
from core.config import END_MARKER, START_MARKER
from prompts import (
...
class PromptInfo:
    ...
    def __init__(self, prompt_data: dict, lang: str = "en"):
        ...
        self.template: str | None = prompt_data.get("full_template")
...
class PromptService:
    ...
    def get_all_prompts(self, lang: str | None = None) -> list[PromptInfo]:
    ...
    def get_prompt_by_id(self, prompt_id: str, lang: str | None = None) -> PromptInfo | None:
    ...
    def get_prompts_by_category(self, category: str, lang: str | None = None) -> list[PromptInfo]:
        """Get all prompts in a specific category."""
        lang = lang or self._default_lang
        prompts = get_prompts_by_category(category)
        return [PromptInfo(p, lang) for p in prompts]

    def get_categories(self, lang: str | None = None) -> list[CategoryInfo]:
        """Get all available categories with localized names."""
        lang = lang or self._default_lang
        categories = get_categories()
        return [CategoryInfo(c, lang) for c in categories]

    def get_prompt_names(self, lang: str | None = None) -> dict[str, str]:
    ...
    def get_prompt_for_api(self, prompt_id: str, lang: str | None = None) -> dict | None:
        """
    ...
    def get_all_for_api(self, lang: str | None = None) -> list[dict]:
